...
async function select_comments(article_id, comment_level) {
  const sql = `SELECT * from comment where article_id = ${article_id} ORDER BY top_status desc ,praise_num desc`
  var list = []
  await query(sql, [])
    .then(res => {
      if (res.length !== 0) {
...
async function select_replys(article_id, comment_level) {
  const sql = `SELECT * from comment where  article_id = ${article_id} ORDER BY create_time desc `
